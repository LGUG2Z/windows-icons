...
fn get_icon_file_path(
    ...
    if desktop_icon_path.exists() {
        ...
        let icon_scale_path =
            match_icon_path(&icon_full_path, icon_matcher).unwrap_or(icon_full_path.to_string());
...
fn file_stem_contains(path_buf: &Path, text: &str) -> bool {
    path_buf
        .file_stem()
        .unwrap_or_default()
        .to_str()
...
fn match_icon_path(icon_path: &str, icon_matcher: &IconMatcher) -> Option<String> {
    ...
    let mut matching_files: Vec<PathBuf> = folder_path
        .read_dir()
        .ok()?
        .filter_map(|de| de.ok())
        .map(|de| de.path())
...
fn reduce_to_best_scale(matching_files: &[PathBuf], scale: i16) -> Option<Vec<PathBuf>> {
    ...
    let paths_to_remove: Vec<&PathBuf> = removal_candidates
        .iter()
        .filter(|p| !file_stem_contains(p, &format!("scale-{best_match}")))
        .copied()
        .collect();
    ...
    let mut all_paths = matching_files.to_owned();
